...
% TODO: pagebreak
%\pagebreak[4]
Also it is possible to define members and to provide default values outside of the constructor, as well as to mark properties as \texttt{readonly}, which prohibits reassignments at compile time:
\begin{JsCode}[numbers=none]
class Person implements Human {
...
\end{program}
\begin{program}[t]
\caption{The code below shows an ECMAScript proposal for \texttt{Builtin.is} and \texttt{Builtin.of}, where the former command determines if two values point to the same built-in constructor, and the latter can obtain the type of primitive and built-in values~\cites{DynamicTypeChecks:Rauschmayer:2017, BuiltinProposalRepo}, in contrast to the existing \texttt{typeof} operator which can get the primitive type only.}
