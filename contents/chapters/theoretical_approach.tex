...
In TypeScript it is possible to perform a special kind of type cast, called \emph{type assertion}, as described in Section~\ref{sec:typescript}. While the compiler will trigger an error, when trying to assert an incompatible type---e.g., \text{asserting} a string as a number---there exists a special case to bypass static type checks for a given variable or value entirely. If, e.g., a variable is annotated or asserted with the \emph{Any} type, type checking and type inference will be disabled for this part of the code. The TypeScript documentation describes this type as 
\begin{quote}
...
\end{JsCode}
\end{program}
To enable type checks for the runtime, the enum has to be described with its members.%, which in the case of Program~\ref{prog:enum-compiled} is one member with the value of the number zero.
